...
# Ensure the containing directory 'Source Code' is on the python import path
...
# Resolve paths relative to the project root directory (parent of Source Code)
...
def generate_synthetic_data(num_records=1000):
    ...
    df_app.to_csv(os.path.join(DATA_DIR, 'application_record.csv'), index=False)
    ...
    df_credit.to_csv(os.path.join(DATA_DIR, 'credit_record.csv'), index=False)
    print(f"Synthetic datasets successfully created in {DATA_DIR}.")

# Clean & Preprocess Data
def load_and_preprocess_data():
    if not os.path.exists(os.path.join(DATA_DIR, 'application_record.csv')) or not os.path.exists(os.path.join(DATA_DIR, 'credit_record.csv')):
...
def run_pipeline():
    ...
    for name, clf in models.items():
        ...
        model_filename = os.path.join(MODELS_DIR, f"{clean_name}_pipeline.pkl")
        joblib.dump(pipeline, model_filename)
        results[name]['path'] = f"Models/{clean_name}_pipeline.pkl"
